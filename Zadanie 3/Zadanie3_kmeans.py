...
def kmeans(array, k):
    ...
    while not np.array_equal(centroids, new_centroids):
        for j in range(len(array)):
            ...
            clusters_dst[minposition].append(min(distanses))
        for u in range(k):
            if len(clusters[u]) > 1:
                new_centroids[u] = np.mean(clusters[u], axis=0)
            else:
        ...
        if np.array_equal(centroids, new_centroids):
            ...
            new_centroids = [[0.0 for _ in range(k)] for _ in range(k)]
            li += 1
    wcss = np.round(sum(sum(cluster) for cluster in clusters_dst), 3)
    print(str(wcss) + " k = " + str(k))
    return new_clusters, centroids, li, wcss
...
def all_plots():
    ...
    for o in range(2, 11):
        for i in range(6):
            ...
            colors = ["red", "blue", "green", "pink", "purple", "yellow", "grey", "orange", "brown", "black"]
            clusters = result[0]
            centroids = result[1]
            max_iter = result[2]
            wcss = result[3]
